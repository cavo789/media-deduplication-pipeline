...
| `org.opencontainers.image.vendor` | Derive from `git config user.name` — never hardcode a personal name in this public file. |
| `org.opencontainers.image.licenses` | Derive from the repository's LICENSE file; if there is none, ask — never assume one. |
| `org.opencontainers.image.authors` | Derive from `git config user.email` — never hardcode a personal address in this public file. |

Title/version/description are judgment calls — say what each was derived from so the user can
correct it in the same review pass; vendor/authors come from git config and licenses from the
LICENSE file — state the source, and ask only when it is missing.

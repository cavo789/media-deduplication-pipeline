...
in `--prefer` (or `folders.preferred`) and audit again, or [decide pair by pair in the
report](#going-further).
...
**Decide in the report** — not the folder you want to keep? In the *folder pairs* table of an
audit report, each pair has a *Your decision* list: *Swap the folders* keeps the copies of the
second folder and deletes those of the first one; *Leave alone* deletes nothing of that pair.
Your browser remembers the choices. *Download decisions.json*, save it in the folder mounted on
`/reports`, and add `--decisions decisions.json` to the `clean` command above. The page itself
never deletes anything: `clean` audits again, applies your decisions, shows the pairs that
result and asks before cleaning, with every safeguard (byte comparison, journal, `undo`). The
file is refused if other folders are mounted, if a decided pair no longer exists (files changed
since the report), or if a swap would delete the copies of a protected folder.
...
- **Its photo is kept**: between identical copies, the one with a sidecar is kept, so it keeps
  ...
  their own sidecar, the other rules choose among them.
...
another copy *with* its edits, name its folder in `--prefer`.
...
| Which copy is kept | Deterministic: a protected folder, then your preferred folders (in order), then the copy with a [sidecar](#sidecar-files) (its edits), a name that does not look like a copy (`IMG (1).jpg`, `IMG - Copie.jpg`, …), a name someone chose rather than one generated by a camera or an app (`Marie et Paul.jpg` rather than `IMG_1234.jpg`), a folder someone named rather than a generic one (`Vacances 2019` rather than `DCIM\100CANON`), the oldest date, the shortest path. The report says, for each group, which rule decided. Your [decisions in the report](#going-further) (`--decisions`) come on top. |
...
- **Run with `-it`**: without a terminal, `clean` cannot ask for confirmation (use `--yes`) and
  colours are off.
- **Folders the tool cannot write to**: when a folder given to `-v` does not exist yet, Docker
  creates it for `root`, and the tool (which does not run as `root`) cannot write there. The
  command then stops before the analysis and names the folder. Create your folders before
  `docker run` and, from WSL or Linux, add `--user "$(id -u):$(id -g)"`; a folder Docker already
  created is yours again with `sudo chown "$(id -u):$(id -g)" <folder>`. A `:ro` on `/journal`,
  `/quarantine`, `/reports` or `/cache` stops it the same way. Only `config.toml` is optional:
  it is then not created. If the report alone fails after an audit, a warning says so and the

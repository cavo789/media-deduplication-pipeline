...
| `plan/` | `KeepPolicy` (protected > preferred > not a copy name > name not generated by a device/app > folder not generic > oldest > shortest path > alphabetical; the decisive criterion is recorded as `KeepReason`) and `build_plan`; `similar` finds burst series (`bursts`, listed only) then near duplicates (`near`, multi-index hashing; `clean --tier near` quarantines them, never deletes). |

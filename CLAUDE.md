...
| Package | Role |
|---|---|
| `cli/` | Typer commands (`audit`, `crosscheck`, `clean`, `undo`, `history`, `reports`, `purge`, `config`); thin: build a settings layer, call a service, display. `cli/app.py` builds the app **after** the locale is installed (`__main__.py`). |
| `services/` | Use cases: `AuditService`, `CleanService`, `cross_check`, `undo_run`, `write_report`; `Runtime` bundles settings, locations, mount table, output. |

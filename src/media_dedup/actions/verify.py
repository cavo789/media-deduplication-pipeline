"""Last-moment checks: the kept copy must still exist and still be identical."""
...
def removal_blocker(keeper: Path, candidate: Path, size: int) -> str | None:
    ...
    Safe means: both files exist, still have the scanned size, and are equal byte for
    byte — so deleting `candidate` never loses data.
